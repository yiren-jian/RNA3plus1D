...
class C3D(nn.Module):
    ...
    def __init__(self, num_classes, channels, length, batch_norm=False):
        super(C3D, self).__init__()
        self.batch_norm = batch_norm

        self.conv1a = nn.Conv3d(channels*length, 64, kernel_size=(3, 3, 3), padding=(1, 1, 1), stride=(1, 1, 1), dilation=(1, 1, 1), bias=True )
...
if __name__ == "__main__":
    inputs = torch.rand(6, 3, 128, 32, 32, 32)   #### bsz=1, one positive and one negative
    y = torch.tensor([0,0,0,1,1,1])     #### label, 0 and 1 for two RNAs (positive and negative).
    net = C3D(num_classes=2, channels=3, length=128, batch_norm=True)
    print('  Total params in share model: %.2fM' % (sum(p.numel() for p in net.parameters())/1000000.0))

    outputs = net.forward(inputs)
    print(outputs)

    loss = net.forward(inputs, y)
    print(loss)
